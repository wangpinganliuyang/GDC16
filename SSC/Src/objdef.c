...
/*ECATCHANGE_START(V5.11) OBJ1*/
    ...
    UINT16 Index = pObjEntry->Index;

    if(subindex > 0)
    {
        /*subindex 1 has an offset of 16Bit (even if Si0 is only an UINT8) */
    	if((Index == 0x1018) | (Index == 0x10F1) | (Index == 0x1600) | (Index == 0x1601) | (Index == 0x1602)
    			| (Index == 0x1A00) | (Index == 0x1A01) | (Index == 0x1A02)
				| (Index == 0x607D) | (Index == 0xF010) | (Index == 0xF030) | (Index == 0xF050))
    	{
    		bitOffset +=32;
    	}
    	else
...
/*ECATCHANGE_END(V5.11) OBJ1*/
    ...
    for (i = 1; i <= subindex; i++)
    {
        ...
        {
        case    DEFTYPE_UNSIGNED8://添加的
        case    DEFTYPE_INTEGER8://添加的
...
/*ECATCHANGE_START(V5.11) SDO9*/
            if (i < subindex)
            {
                if((pEntry->DataType == DEFTYPE_UNICODE_STRING)//deftype unicode string
                    ||(pEntry->DataType == DEFTYPE_ARRAY_OF_INT))//deftype array o int
                {
...
/*ET9300 Project Handler :(#if (BIG_ENDIAN_16BIT || BIG_ENDIAN_FORMAT)) lines 1079 to 1081 deleted*/
                    pData++;
                    pVarPtr++;
                }
...
//            OBJTOMBXSTRCPY(pData,pVarPtr,size-4);//修改
            OBJTOMBXSTRCPY(pData,pVarPtr,((size-4+1)>>1));//修改 byte enum
            }
        }
        else
        {
            ...
            for (i = subindex; i <= lastSubindex; i++)
            {
                ...
                pVarPtr += (bitOffset >> 4);//对32位的内存对齐有影响，修改   包含有32位数据的对象，单独来下
...
//                                OBJTOMBXMEMCPY(pData, pVarPtr, BIT2BYTE(pEntry->BitLength));//
                                OBJTOMBXMEMCPY(pData, pVarPtr, ((BIT2BYTE(pEntry->BitLength)+1)>>1));
...
//                                OBJTOMBXSTRCPY(pData, pVarPtr, BIT2BYTE(pEntry->BitLength));//修改
                                //暂未想到更换的方法，根据索引之间赋值，需要先将字符串转成16进制,同时修改相应定义的对象字节长度
                                if(index == 0x1008)
                                {
                                	 pData[0]=0x4447;
                                	 pData[1]=0x3143;
...
//                            OBJTOMBXSTRCPY(pVarPtr, pData, BIT2BYTE(pEntry->BitLength));//修改
                            OBJTOMBXSTRCPY(pVarPtr, pData, ((BIT2BYTE(pEntry->BitLength)+1)>>1));//修改
...
//                            OBJTOMBXMEMCPY(pVarPtr, pData, BIT2BYTE(pEntry->BitLength));//修改
                            OBJTOMBXMEMCPY(pVarPtr, pData, BIT2WORD(pEntry->BitLength));//修改
/*ECATCHANGE_START(V5.11) SDO2*/
                            pData += BIT2WORD((pEntry->BitLength) & ~0xF);
/*ECATCHANGE_END(V5.11) SDO2*/
