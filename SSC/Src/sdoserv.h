...
#define INITSDO_HEADER_SIZE     ((SIZEOF(TINITSDOHEADER))<<1) /**< \brief Size of the SDO header*/
...
/*ET9300 Project Handler :(#if MBX_16BIT_ACCESS #else) lines 170 to 181 deleted*/
}MBX_STRUCT_PACKED_END
TSDOSEGHEADERDATA;
...
//#define ABORT_NORM_RES_SIZE    (SIZEOF(TABORTSDOTRANSFERREQMBX) - SIZEOF(TMBXHEADER))/**< \brief Size of the SDO Abort datagram (without the mailbox header)*///大小统一按照byte方式
#define ABORT_NORM_RES_SIZE    ((SIZEOF(TABORTSDOTRANSFERREQMBX) - SIZEOF(TMBXHEADER))<<1)/**< \brief Size of the SDO Abort datagram (without the mailbox header)*/
/** @}*/
...
#define SDO_INFO_OBJ_DESC_SIZE      (SIZEOF(TSDOINFOOBJDESC)+2) /**< \brief Object description size*/
...
//#define SDO_INFO_OBJ_DESC_RES_SIZE  SIZEOF(TSDOINFOOBJ)/**< \brief Size of the object description service data*/修改
#define SDO_INFO_OBJ_DESC_RES_SIZE  (SIZEOF(TSDOINFOOBJ))<<1/**< \brief Size of the object description service data*///修改 byte
...
//#define SDO_INFO_ERROR_SIZE     SIZEOF(TSDOINFOERROR)/**< \brief SDO Info error size*/修改
#define SDO_INFO_ERROR_SIZE     ((SIZEOF(TSDOINFOERROR))<<1)/**< \brief SDO Info error size*///修改 byte
