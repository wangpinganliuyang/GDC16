...
UINT64 u64CycleTimeStartValue; /** <\brief contains the timer start value to measure the application cycle (used in freerun and SM2 sync)*/

UINT64 u64MinCycleTimeStartValue; /** <\brief timeout counter in ms to measure the process timings (stored in 0x1C3x)*/

UINT64 u64SystemTimeReadFailure; /** <\brief System time measurement failure (the value is calculated on main init)*/

BOOL bMinCycleTimeMeasurementStarted; /** <\brief Indicates if the min cycle measurement is started*/

UINT64 u64MinCycleTimeValue; /** <\brief tmp value of the min cycle time during measurement*/
...
-----------------------------------------------------------------------------------------*/
/*ECATCHANGE_START(V5.12) ECAT1*/
UINT64 GetSystemTimeDelay(UINT64 u64StartTime);
...
//    HW_EscWriteIsr(((MEM_ADDR *) aPdInputData), nEscAddrInputData, (nPdInputSize>>1) );
    //在此处进行修改
    ...
    if (MEASUREMENT_ACTIVE)
    {
        u64TimeValue = GetSystemTimeDelay(u64TimeValue);

#if (PD_INPUT_CALC_AND_COPY_TIME == 0)
        if (sSyncManInPar.u32CalcAndCopyTime < u64TimeValue)
        {
            sSyncManInPar.u32CalcAndCopyTime = u64TimeValue;
        }
#endif
        ...
        if (bMinCycleTimeMeasurementStarted == TRUE)
        {
            ...
            u64MinCycleTimeValue = u64MinCycleTimeValue + u64TimeValue;


            if (sSyncManOutPar.u32MinCycleTime < u64MinCycleTimeValue)
            {
                    sSyncManOutPar.u32MinCycleTime = u64MinCycleTimeValue;
            }

            if (sSyncManInPar.u32MinCycleTime < u64MinCycleTimeValue)
            {
                    sSyncManInPar.u32MinCycleTime = u64MinCycleTimeValue;
            }
...
void PDO_OutputMapping(void)
{
	...
	/*ECATCHANGE_START(V5.12) ECAT1*/
	   ...
	   if (MEASUREMENT_ACTIVE)
	   {
	      ...
	      u64TimeValue = GetSystemTimeDelay(0);
	      u64MinCycleTimeStartValue = u64TimeValue;

	      bMinCycleTimeMeasurementStarted = TRUE;
	      u64MinCycleTimeValue = 0;
...
//    HW_EscReadIsr(((MEM_ADDR *)aPdOutputData), nEscAddrOutputData, nPdOutputSize );
    ...
    /*ECATCHANGE_START(V5.12) ECAT1*/
        ...
        if (MEASUREMENT_ACTIVE)
        {
           ...
           if (sSyncManOutPar.u32CalcAndCopyTime < u64TimeValue)
           {
              sSyncManOutPar.u32CalcAndCopyTime = u64TimeValue;
           }
    #endif

    #if (MIN_PD_CYCLE_TIME == 0)
           /* add the first part of the min cycle time */
           u64MinCycleTimeValue = u64TimeValue;
    #endif
        }
...
void HandleCycleTimeMeasurement(void)
{
    ...
    {
        if (u64CycleTimeStartValue > 0)
        {
            /* bus cycle completed*/
            u64CycleTimeStartValue = GetSystemTimeDelay(u64CycleTimeStartValue);

            if ((sSyncManOutPar.u32CycleTime == 0) || (sSyncManOutPar.u32CycleTime > u64CycleTimeStartValue))
            {
                    sSyncManOutPar.u32CycleTime = u64CycleTimeStartValue;
            }

            if ((sSyncManInPar.u32CycleTime == 0) || (sSyncManInPar.u32CycleTime > u64CycleTimeStartValue))
            {
                    sSyncManInPar.u32CycleTime = u64CycleTimeStartValue;
            }
        }
        /* get next start value */
        u64CycleTimeStartValue = GetSystemTimeDelay(0);
...
UINT64 GetSystemTimeDelay(UINT64 u64StartTime)
{
   UINT64 u64CurValue = 0;
   UINT64 u64Delta = 0;


   HW_EscReadDDWordIsr(u64CurValue, ESC_SYSTEMTIME_OFFSET);

   if (u64CurValue > 0)
   {
      if (u64StartTime <= u64CurValue)
      {
         u64Delta = u64CurValue - u64StartTime;
      }
      else
      {
         //The 32Bit timer is wrapped around
         u64Delta = u64CurValue + (0xFFFFFFFFFFFFFFFF - u64StartTime);
      }
   ...
   if (u64StartTime > 0)
   {
       ...
       if (u64SystemTimeReadFailure < u64Delta)
       {
           u64Delta = u64Delta - u64SystemTimeReadFailure;
       }
       else
       {
...
/*ECATCHANGE_END(V5.11) EEPROM1*/
    ...
    /*ECATCHANGE_START(V5.12) ECAT1*/
        ...
        bMinCycleTimeMeasurementStarted = FALSE;
        u64CycleTimeStartValue = 0;
        u64MinCycleTimeStartValue = 0;

        u64SystemTimeReadFailure = 0;
        ...
        /* Get the System Time read failure */
        {
           ...
           while (u64Cnt < 1000)
           {
               HW_EscReadDDWordIsr(u64TimeValue, ESC_SYSTEMTIME_OFFSET);
               HW_EscReadDDWordIsr(u64Delta, ESC_SYSTEMTIME_OFFSET);

                if (u64TimeValue <= u64Delta)
                {
                	u64Delta = u64Delta - u64TimeValue;
                }
                else
                {
                    //The 32Bit timer is wrapped around
                	u64Delta = u64Delta + (0xFFFFFFFFFFFFFFFF - u64TimeValue);
                }


                if (u64SystemTimeReadFailure == 0)
                {
                	u64SystemTimeReadFailure = u64Delta;
                }
                else if (u64SystemTimeReadFailure > u64Delta)
                {
                	u64SystemTimeReadFailure = u64Delta;
                }
