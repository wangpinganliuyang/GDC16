...
//        HW_EscReadWord(sm1Activate,(ESC_SYNCMAN_ACTIVE_OFFSET + SIZEOF_SM_REGISTER));
        sm1Activate = ESC_readWordNonISR(ESC_SYNCMAN_ACTIVE_OFFSET + SIZEOF_SM_REGISTER);
...
//       psWriteMbx = (TMBX MBXMEM *) APPL_AllocMailboxBuffer(u16ReceiveMbxSize);
    //修改
    psWriteMbx = (TMBX MBXMEM *) APPL_AllocMailboxBuffer((u16ReceiveMbxSize)>>1);
        if(psWriteMbx == NULL)
        {
...
//            psWriteMbx = (TMBX MBXMEM *) APPL_AllocMailboxBuffer(10); /* a mailbox error datagram length
            //修改，8位-16位
            psWriteMbx = (TMBX MBXMEM *) APPL_AllocMailboxBuffer(5); // a mailbox error datagram length
            if(psWriteMbx == NULL)
            {
...
UINT8 MailboxServiceInd(TMBX MBXMEM *pMbx)
{
    ...
    switch ( (pMbx->MbxHeader.Flags & MBX_MASK_TYPE) >> MBX_SHIFT_TYPE )
    {
    case MBX_TYPE_COE:
...
{
    UINT8 result = 0;
    UINT8 mbxCounter = pMbx->MbxHeader.Flags >> MBX_SHIFT_COUNTER;
    ...
    if(MbxLen > MAX_MBX_SIZE)
    {
        ...
        pMbx->MbxHeader.Flags        &= ~MBX_MASK_TYPE;
...
UINT8 MBX_MailboxSendReq( TMBX MBXMEM * pMbx, UINT8 flags )
{
    ...
    pMbx->MbxHeader.Flags &= ~MBX_MASK_COUNTER;
    ...
    pMbx->MbxHeader.Flags |= u8MbxReadCounter << MBX_SHIFT_COUNTER;
...
{
    UINT16 mbxLen;
    UINT16 adr_u16EscAddrReceiveMbx = 0;//修改程序添加的过程变量
    UINT16 psWriteMbxi = 0;//修改程序添加的过程变量
...
//    mbxLen = SWAPWORD(mbxLen);
    mbxLen = ESC_readWordNonISR(u16EscAddrReceiveMbx);
...
//        TMBX MBXMEM *pMbx = (TMBX MBXMEM *) APPL_AllocMailboxBuffer(10); /* a mailbox error datagram length
    	//修改：8位--16位
    	TMBX MBXMEM *pMbx = (TMBX MBXMEM *) APPL_AllocMailboxBuffer(5);//修改
...
//        psWriteMbx = (TMBX MBXMEM *) APPL_AllocMailboxBuffer(u16ReceiveMbxSize);
        //修改：原8位--16位存储
        psWriteMbx = (TMBX MBXMEM *) APPL_AllocMailboxBuffer((u16ReceiveMbxSize)>>1);//修改
...
//        HW_EscReadMbxMem((MEM_ADDR MBXMEM *) psWriteMbx,u16EscAddrReceiveMbx,mbxLen);
        //此次做了修改，为保存读写对齐
        adr_u16EscAddrReceiveMbx = u16EscAddrReceiveMbx;
        psWriteMbx->MbxHeader.Length = ESC_readWordNonISR(adr_u16EscAddrReceiveMbx);
        adr_u16EscAddrReceiveMbx += 2;
        psWriteMbx->MbxHeader.Address = ESC_readWordNonISR(adr_u16EscAddrReceiveMbx);
        adr_u16EscAddrReceiveMbx += 2;
        psWriteMbx->MbxHeader.Flags = ESC_readWordNonISR(adr_u16EscAddrReceiveMbx);
        adr_u16EscAddrReceiveMbx += 2;
        for(psWriteMbxi=0; psWriteMbxi<((mbxLen-6+1)>>1); psWriteMbxi++)
        {
        	psWriteMbx->Data[psWriteMbxi] = ESC_readWordNonISR(adr_u16EscAddrReceiveMbx);
        	adr_u16EscAddrReceiveMbx += 2;
        }
        //本段修改完成
...
//        smstate = SWAPWORD(smstate);
        smstate = ESC_readWordNonISR(ESC_SYNCMAN_CONTROL_OFFSET);
...
//            HW_EscReadWord(u16dummy,(u16EscAddrReceiveMbx + u16ReceiveMbxSize - 2));
            u16dummy = ESC_readWordNonISR((u16EscAddrReceiveMbx + u16ReceiveMbxSize - 2));
...
{
	UINT16 adr_u16EscAddrSendMbx = 0;
	UINT16 New_Data = 0;
	UINT16 i_mbxSize = 0;
    if ( (nAlStatus & STATE_MASK) == STATE_INIT)
    {
...
//        HW_EscWriteMbxMem((MEM_ADDR *)pMbx, u16EscAddrSendMbx, (mbxSize + MBX_HEADER_SIZE));
        ...
        //因字节对齐问题，修改代码
        adr_u16EscAddrSendMbx = u16EscAddrSendMbx;
        New_Data = pMbx->MbxHeader.Length;
        ESC_writeWordNonISR(New_Data, adr_u16EscAddrSendMbx);
        adr_u16EscAddrSendMbx += 2;
        New_Data = pMbx->MbxHeader.Address;
        ESC_writeWordNonISR(New_Data, adr_u16EscAddrSendMbx);
        adr_u16EscAddrSendMbx += 2;
        New_Data = pMbx->MbxHeader.Flags;
        ESC_writeWordNonISR(New_Data, adr_u16EscAddrSendMbx);
        adr_u16EscAddrSendMbx += 2;
        for (i_mbxSize=0; i_mbxSize <((mbxSize+1)>>1); i_mbxSize++)
        {
        	New_Data = pMbx->Data[i_mbxSize];
        	ESC_writeWordNonISR(New_Data, adr_u16EscAddrSendMbx);
        	adr_u16EscAddrSendMbx += 2;
        }
        //本段代码修改完毕，完成地址的匹配
...
/*ECATCHANGE_START(V5.11) ECAT7*/
//        smstate = SWAPWORD(smstate);
        smstate = ESC_readWordNonISR(ESC_SYNCMAN_CONTROL_OFFSET + SIZEOF_SM_REGISTER);
/*ECATCHANGE_END(V5.11) ECAT7*/
...
//                MEMCPY((UINT16 *)&u16dummy,(((UINT8 *)pMbx) + LastDataAddress),(2 - BytesLeft));//修改
                MEMCPY((UINT16 *)&u16dummy,(((UINT8 *)pMbx) + LastDataAddress),1);//修改
            }
