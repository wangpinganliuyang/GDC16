...
//    HW_EscReadWord(mask, ESC_AL_EVENTMASK_OFFSET);
    mask = ESC_readWordNonISR(ESC_AL_EVENTMASK_OFFSET);
...
//       HW_EscReadWord(TmpVar, ESC_EEPROM_CONTROL_OFFSET);
       TmpVar = ESC_readWordNonISR(ESC_EEPROM_CONTROL_OFFSET);
...
//    HW_EscRead((MEM_ADDR *)&SyncManInfo, ESC_SYNCMAN_REG_OFFSET + (channel * SIZEOF_SM_REGISTER), SIZEOF_SM_REGISTER );
	UINT16 addr = 0;
	addr = ESC_SYNCMAN_REG_OFFSET + (channel * SIZEOF_SM_REGISTER);
	SyncManInfo.PhysicalStartAddress = ESC_readWordNonISR(addr);
	addr += 2;
	SyncManInfo.Length = ESC_readWordNonISR(addr);
	addr += 2;
	SyncManInfo.Settings[0] = ESC_readWordNonISR(addr);
	addr += 2;
	SyncManInfo.Settings[1] = ESC_readWordNonISR(addr);
...
//    HW_EscWriteWord(smStatus,Offset);
    ESC_writeWordNonISR(smStatus,Offset);
    
    /*wait until SyncManager is disabled*/
    do
    {
//        HW_EscReadWord(smStatus, Offset);
    	smStatus = ESC_readWordNonISR(Offset);
...
UINT16 StartInputHandler(void)
{
    ...
    UINT16        dcControl;
    UINT16     wdiv = 0;
    UINT16 cycleTimeSync01 = 0;
    UINT16 cycleTimeSync02 = 0;
    UINT16 cycleTimeSync11 = 0;
    UINT16 cycleTimeSync12 = 0;
...
//    dcControl = SWAPWORD(dcControl);
    dcControl = ESC_readWordNonISR(ESC_DC_UNIT_CONTROL_OFFSET);

    // Cycle time for Sync0
//        HW_EscReadDWord(cycleTimeSync0, ESC_DC_SYNC0_CYCLETIME_OFFSET);
//        cycleTimeSync0 = SWAPDWORD(cycleTimeSync0);
    cycleTimeSync01 = ESC_readWordNonISR(ESC_DC_SYNC0_CYCLETIME_OFFSET);
    cycleTimeSync02 = ESC_readWordNonISR((ESC_DC_SYNC0_CYCLETIME_OFFSET+2));
    cycleTimeSync0 = (((UINT32)cycleTimeSync02)<<16) + cycleTimeSync01;

    // Cycle time for Sync1
//        HW_EscReadDWord(cycleTimeSync1, ESC_DC_SYNC1_CYCLETIME_OFFSET);
//        cycleTimeSync1 = SWAPDWORD(cycleTimeSync1);
    cycleTimeSync11 = ESC_readWordNonISR(ESC_DC_SYNC1_CYCLETIME_OFFSET);
    cycleTimeSync12 = ESC_readWordNonISR((ESC_DC_SYNC1_CYCLETIME_OFFSET+2));
    cycleTimeSync1 = (((UINT32)cycleTimeSync12)<<16) + cycleTimeSync11;
...
//     	bEscIntEnabled = FALSE;
        ...
        || (SyncType0x1C33 == SYNCTYPE_DCSYNC0) || (SyncType0x1C33 == SYNCTYPE_DCSYNC1))/* Sync to Sync0 or Sync1 is enabled*/
    {
            ...
            bDcSyncActive = TRUE;//同步修改
//    	bDcSyncActive = FALSE;//修改
...
//    wd = SWAPWORD(wd);
    wd = ESC_readWordNonISR(ESC_PD_WD_TIME);
/*ECATCHANGE_END(V5.11) ECAT4*/
...
//    wdiv = SWAPWORD(wdiv);
    	wdiv = ESC_readWordNonISR(ESC_WD_DIVIDER_OFFSET);
        if ( wdiv != 0 )
        {
...
//      WdStatusOK = SWAPWORD(WdStatusOK);
      WdStatusOK = ESC_readWordNonISR(ESC_PD_WD_STATE);
...
//    ALEventReg = SWAPWORD(ALEventReg);
    ALEventReg = ESC_readWordNonISR(ESC_AL_EVENT_OFFSET);
...
//        EscAlControl = SWAPWORD(EscAlControl);
    	EscAlControl = ESC_readWordNonISR(ESC_AL_CONTROL_OFFSET);
