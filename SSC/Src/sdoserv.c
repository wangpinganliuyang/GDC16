...
/* ECATCHANGE_END(V5.11) ECAT7*/
            ...
            if ( abort == 0 )
            {
            	...
            	// Read Data0
            	 ...
            	 // Copy Data1 - DataN
            	    MBXMEMCPY(&pSdoSegData[(nSdoSegBytesToHandle >> 1) + 1],pSdoInd->SdoHeader.Data, ((bytesToSave - 1)>>1) );
            	}
            	else
...
//    SegHeader0 = (pSdoInd->SegHeader) & (0x00FF);//修改
...
//        MBXMEMCPY( pSdoSegRes->SdoHeader.Data, &(((UINT8*)pSdoSegData)[nSdoSegBytesToHandle]), size );//修改
        //word内存对齐方式，修改代码
        //上传同样由nSdoSegBytesToHandle来判断操作
        /*if(!(nSdoSegBytesToHandle & 0x1))//偶数个
        {
        	pSdoSegRes->SdoHeader.Data[0] = (pSdoSegRes->SdoHeader.Data[0] & 0x00FF)+ ((((UINT16 *)pSdoSegData)[(nSdoSegBytesToHandle>>1)])<<8);
        	if(size>1)
        	{
        		for(sizei=0;sizei<((size-1)>>1);sizei++)
        		{
        			Data0 = ((((UINT16 *)pSdoSegData)[(nSdoSegBytesToHandle>>1)+sizei])>>8);
        		    Data1 = ((((UINT16 *)pSdoSegData)[(nSdoSegBytesToHandle>>1)+sizei+1]) & 0x00FF);
        		    pSdoSegRes->SdoHeader.Data[sizei+1] = Data0 +(Data1<<8);
        		}
        	}
        ...
        {
        	pSdoSegRes->SdoHeader.Data[0] = (pSdoSegRes->SdoHeader.Data[0] & 0x00FF)+ (((((UINT16 *)pSdoSegData)[((nSdoSegBytesToHandle-1)>>1)])>>8)<<8);
        	if(size>1)
        	{
        		for(sizei=0;sizei<((size-1)>>1);sizei++)
        		{
        			pSdoSegRes->SdoHeader.Data[sizei+1] = ((UINT16 *)pSdoSegData)[((nSdoSegBytesToHandle-1)>>1)+1+sizei];
        		}
        	}
        }*/
        //修改memcpy函数对齐结束
        ...
        /*Clear SDO header*/
        pSdoSegRes->SdoHeader.SegHeader &= ~SEGHEADER_MASK;//修改
//        pSdoSegRes->SdoHeader.Data[0] &=~(0x00FF);//修改
        if (bSdoSegFollows)
            pSdoSegRes->SdoHeader.SegHeader        |= SDOSERVICE_UPLOADSEGMENTRES | bSdoSegLastToggle;//修改
//        	pSdoSegRes->SdoHeader.Data[0]        |= SDOSERVICE_UPLOADSEGMENTRES | bSdoSegLastToggle;//修改
        else
            pSdoSegRes->SdoHeader.SegHeader        |= SDOSERVICE_UPLOADSEGMENTRES | bSdoSegLastToggle | SEGHEADER_NOMOREFOLLOWS;//修改
//        	pSdoSegRes->SdoHeader.Data[0]        |= SDOSERVICE_UPLOADSEGMENTRES | bSdoSegLastToggle | SEGHEADER_NOMOREFOLLOWS;//修改
        ...
        if ( size < MIN_SEGMENTED_DATA )
        {
            ...
            pSdoSegRes->SdoHeader.SegHeader    |= (MIN_SEGMENTED_DATA - size) << SEGHEADERSHIFT_SEGDATASIZE;//修改
//            pSdoSegRes->SdoHeader.Data[0]    |= (MIN_SEGMENTED_DATA - size) << SEGHEADERSHIFT_SEGDATASIZE;//修改
        }
        else
        {
...
//        pSdoRes->SdoHeader.Sdo[0] &= 0xFF00;
    ...
    if ( abort == 0 )
    {
        ...
        if ( command == SDOSERVICE_INITIATEUPLOADREQ ) /**< \brief SDO upload request*/
        {
...
//                pSdoRes->SdoHeader.Sdo[0]    |=     SDOHEADER_SIZEINDICATOR     |
//                                                                                                        SDOHEADER_TRANSFERTYPE        |
//                                                                                                        completeAccess |
//                                                                                                        ((MAX_EXPEDITED_DATA - ((UINT8)objLength)) << SDOHEADERSHIFT_DATASETSIZE) |
//                                                                                                        SDOSERVICE_INITIATEUPLOADRES;
...
//                                                                 SDOSERVICE_INITIATEUPLOADRES;
        ...
        else if ( command == SDOSERVICE_DOWNLOADSEGMENTREQ )/**< \brief SDO segmented download request*/
        {
...
//            pSdoRes->SdoHeader.Sdo[0]    |= SDOSERVICE_DOWNLOADSEGMENTRES;
        }
        else if ( command != SDOSERVICE_UPLOADSEGMENTREQ ) /**< \brief SDO segmented upload request*/
        {
            /* Download response */
            pSdoRes->MbxHeader.Length         = DOWNLOAD_NORM_RES_SIZE;
            pSdoRes->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET]    |= SDOSERVICE_INITIATEDOWNLOADRES;
//            pSdoRes->SdoHeader.Sdo[0]    |= SDOSERVICE_INITIATEDOWNLOADRES;
        }
    }
    else
    {
...
//        pSdoRes->SdoHeader.Sdo[0]    = (((pSdoRes->SdoHeader.Sdo[0])>>8)<<8) + SDOSERVICE_ABORTTRANSFER;//修改
...
//    UINT8 sdoHeader = Sdo0 & SDOHEADER_COMMANDMASK;
    ...
    switch (command)
    {
    case SDOSERVICE_INITIATEDOWNLOADREQ:/**< \brief SDO download request*/
    case SDOSERVICE_INITIATEUPLOADREQ:// /**< \brief SDO upload request*/
        ...
        if ((index >=0x2000) & (index <= 0x3000))
        {
        	...
        	UINT32 *pData = NULL;
        	{
        		...
        		if(command == SDOSERVICE_INITIATEUPLOADREQ)/**< \brief SDO upload request*/
        		{
        			...
        			Shared_Ram_cpu2_WR_Upload(index);
        		}
        		else
        		{
        	...
        	if( command == SDOSERVICE_INITIATEDOWNLOADREQ )/**< \brief SDO download request*/
        	{
        		/* SDO Download */
        		if(bTransferType)
        		{
            ...
            if ( abort == 0 )
            {
            	if ( command == SDOSERVICE_INITIATEUPLOADREQ )/**< \brief SDO upload request*/
            	{
            		...
            		//step1 读取GDC参数，CPU2-CPU1写，读GDC索引参数
            		//step2 CPU1-CPU2读GDC索引参数
                    pData[0] = GDCV05_CPU2_R.GDCIndexValue;
                    pData += 4;
                ...
                else
                {
                	...
                	GDCV05_CPU2_WR.GDCIndexValue = (UINT32 *)pData;
        ...
        if ( pObjEntry )
        {
            ...
            if ( abort == 0 )
            {
                if ( segTransfer )
                {
                    ...
                    pSdoSegData = (UINT16 VARMEM *) ALLOCMEM( ROUNDUPBYTE2WORD(nSdoSegCompleteSize) );//1byte站一个word
...
/*ECATCHANGE_END(V5.11) SDO4*/
                    ...
                    else
                    {
                        if ( command == SDOSERVICE_INITIATEUPLOADREQ )/**< \brief SDO upload request*/
                        {
                            ...
                            if ( abort == 0 )
                            {
                 //               MBXMEMCPY( ((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoInd)->Data, pSdoSegData, dataSize );//修改
                                MBXMEMCPY( ((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoInd)->Data, pSdoSegData, ((dataSize+1)>>1) );//修改
                                nSdoSegService    = SDOSERVICE_UPLOADSEGMENTREQ;/**< \brief SDO segmented upload request*/
                            }
                            else if ( abort == ABORTIDX_WORKING )
                            {
                                ...
                                /*update command field*/
                                pSdoResStored->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET]   &= ~SDOHEADER_COMMANDMASK;
//                                pSdoResStored->SdoHeader.Sdo[0]   &= ~(0x00FF);//修改
                                pSdoResStored->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET]   |= (sdoHeader & (SDOHEADER_COMPLETEACCESS | SDOHEADER_COMMAND));
//                                pSdoResStored->SdoHeader.Sdo[0]   |= ((sdoHeader & (SDOHEADER_COMPLETEACCESS | SDOHEADER_COMMAND)));
                                nSdoSegService    = SDOSERVICE_UPLOADSEGMENTREQ;/**< \brief SDO segmented upload request*/
                                return 0;
                            }
                        }
                        ...
                        {
                            /* Segmented Download */
                 //           MBXMEMCPY( pSdoSegData, ((TINITSDODOWNLOADNORMREQMBX MBXMEM *) pSdoInd)->Data, mbxSize-DOWNLOAD_NORM_REQ_SIZE );//需要修改
                            MBXMEMCPY( pSdoSegData, ((TINITSDODOWNLOADNORMREQMBX MBXMEM *) pSdoInd)->Data, ((mbxSize-DOWNLOAD_NORM_REQ_SIZE+1)>>1) );//需要修改
                ...
                else
                {
                    ...
                    if ( command == SDOSERVICE_INITIATEUPLOADREQ )/**< \brief SDO upload request*/
                    {
                        ...
                        if ( abort == ABORTIDX_WORKING )
                        {
                            ...
                            /*update command field*/
                            pSdoResStored->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET] &= ~SDOHEADER_COMMANDMASK;
//                            pSdoResStored->SdoHeader.Sdo[0] &= ~(0x00FF);
                            pSdoResStored->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET]    |= (sdoHeader & (SDOHEADER_COMPLETEACCESS | SDOHEADER_COMMAND));
//                            pSdoResStored->SdoHeader.Sdo[0]    |= ((sdoHeader & (SDOHEADER_COMPLETEACCESS | SDOHEADER_COMMAND)));
                            return 0;
                        }
...
//                            pSdoResStored->SdoHeader.Sdo[0]    |= ((sdoHeader & (SDOHEADER_COMPLETEACCESS | SDOHEADER_COMMAND)));
    ...
    case SDOSERVICE_DOWNLOADSEGMENTREQ: /**< \brief SDO segmented download request*/
    case SDOSERVICE_UPLOADSEGMENTREQ:/**< \brief SDO segmented upload request*/
...
//        UINT8 command = ((pSdoResStored->SdoHeader.Sdo[0])) & SDOHEADER_COMMAND;//修改
        UINT8 completeAccess = pSdoResStored->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET] & SDOHEADER_COMPLETEACCESS;
//        UINT8 completeAccess = ((pSdoResStored->SdoHeader.Sdo[0])) & SDOHEADER_COMPLETEACCESS;
...
//                MBXMEMCPY(((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoResStored)->Data, pData, dataSize);//修改
                MBXMEMCPY(((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoResStored)->Data, pData, ((dataSize+1)>>1));//修改
                nSdoSegBytesToHandle = dataSize;
            }
...
//                    MBXMEMCPY(((TINITSDOUPLOADEXPRESMBX MBXMEM *) pSdoResStored)->Data, pData, objLength);//修改
//                    MBXMEMCPY(((TINITSDOUPLOADEXPRESMBX MBXMEM *) pSdoResStored)->Data, pData, ((objLength)>>1));//修改
                    for(objLengthi =0;objLengthi<((objLength+1)>>1);objLengthi++)//子索引为0objlength为1
                    {
                    	((TINITSDOUPLOADEXPRESMBX MBXMEM *) pSdoResStored)->Data[objLengthi] = pData[objLengthi];
                    }
            ...
            else
            {
                ...
                {
                    /* the data is not in the response buffer yet, it shall be copied */
//                    MBXMEMCPY(((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoResStored)->Data, pData, objLength);//修改
//                    MBXMEMCPY(((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoResStored)->Data, pData, ((objLength)>>1));
                    for(objLengthi =0;objLengthi<((objLength)>>1);objLengthi++)
                    {
                       	((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoResStored)->Data[objLengthi] = pData[objLengthi];
                    }
                }
            }
        }
...
UINT8 SDOS_SdoInfoInd( TSDOINFORMATION MBXMEM *pSdoInfoInd )
{
    ...
    if ( opCode == SDOINFOSERVICE_ENTRYDESCRIPTION_Q )//get object description请求
    {
    ...
    switch ( opCode )
    {
    case SDOINFOSERVICE_OBJDICTIONARYLIST_Q://get od list请求
 ...
 //           MBXMEMCPY(aSdoInfoHeader, pSdoInfoInd, SDO_INFO_HEADER_BYTE_SIZE);
            //修改：结构体对齐问题
        	aSdoInfoHeader[0] = pSdoInfoInd->MbxHeader.Length;
        	aSdoInfoHeader[1] = pSdoInfoInd->MbxHeader.Address;
        	aSdoInfoHeader[2] = pSdoInfoInd->MbxHeader.Flags;
        	aSdoInfoHeader[3] = pSdoInfoInd->CoeHeader;
        	aSdoInfoHeader[4] = pSdoInfoInd->SdoHeader.InfoHead;
        	aSdoInfoHeader[5] = pSdoInfoInd->SdoHeader.FragmentsLeft;
        	aSdoInfoHeader[6] = pSdoInfoInd->SdoHeader.Data.List.ListType;
        	//修改完成
            if ( listType-- == 0 )//在5个不同的列表中得到对象的数量
            {
    ...
    case SDOINFOSERVICE_OBJDESCRIPTION_Q://get object description请求
    case SDOINFOSERVICE_ENTRYDESCRIPTION_Q://get entry description请求
        ...
        else
        {
            ...
            if ( pObjEntry )
            {
                /* object exists */
                UINT16 size = 0;
                if ( opCode == SDOINFOSERVICE_OBJDESCRIPTION_Q )//get object description请求
                {
                    /* object description is requested */
//                    OBJTOMBXMEMCPY(&pSdoInfoInd->SdoHeader.Data.Obj.Res, OBJ_GetObjDesc(pObjEntry), SDO_INFO_OBJ_DESC_SIZE);//此处正确
                    OBJTOMBXMEMCPY(&pSdoInfoInd->SdoHeader.Data.Obj.Res, OBJ_GetObjDesc(pObjEntry), ((SDO_INFO_OBJ_DESC_SIZE)>>1));//此处正确
...
//                            size = 12 + SIZEOF_SDOINFO + SIZEOF(TSDOINFOENTRY); // 12: Length of "SubIndex 000"修改
                            size = 12 + SIZEOF_SDOINFO + ((SIZEOF(TSDOINFOENTRY))<<1); // 12: Length of "SubIndex 000"修改 byte
                        }
                        else
                        {
...
//                            size = OBJ_GetDesc(index, subindex, pObjEntry, NULL) + SIZEOF_SDOINFO + SIZEOF(TSDOINFOENTRY);//修改
                            size = OBJ_GetDesc(index, subindex, pObjEntry, NULL) + SIZEOF_SDOINFO + ((SIZEOF(TSDOINFOENTRY))<<1);//修改 byte
                            if ( size > (u16SendMbxSize - MBX_HEADER_SIZE) )
                            {
                                /* size of the object description does not fit in the mailbox,
                                the object description will be sent without the name */
//                                size =  SIZEOF_SDOINFO + SIZEOF(TSDOINFOENTRY);//修改
                            	size =  SIZEOF_SDOINFO + ((SIZEOF(TSDOINFOENTRY))<<1);//修改 byte
                            }
                            else
                            {
                                /* object description fits in the mailbox, get the name of the entry */
//                                size = OBJ_GetDesc(index, subindex, pObjEntry, ((UINT16 MBXMEM *) &(&pSdoInfoInd->SdoHeader.Data.Entry.Res)[1])) + SIZEOF_SDOINFO + SIZEOF(TSDOINFOENTRY);//修改
                                size = OBJ_GetDesc(index, subindex, pObjEntry, ((UINT16 MBXMEM *) &(&pSdoInfoInd->SdoHeader.Data.Entry.Res)[1])) + SIZEOF_SDOINFO + ((SIZEOF(TSDOINFOENTRY))<<1);//修改byte
                            }
                        }
                    }
                    else
    ...
    if ( abort )
    {
        ...
        pSdoInfoInd->SdoHeader.FragmentsLeft = 0;
        pSdoInfoInd->SdoHeader.Data.Error.ErrorCodei[0] = (UINT16)(cAbortCode[abort] & 0x0000FFFF);
        pSdoInfoInd->SdoHeader.Data.Error.ErrorCodei[1] = (UINT16)(cAbortCode[abort]>>16);
